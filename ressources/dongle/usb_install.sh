...
touch /tmp/dependancy_onewire_in_progress
if [ $UID -ne 0 ]
then
        echo "L'utilisateur doit être root (ou utiliser sudo)."
        exit 1
fi
echo 0 > /tmp/dependancy_onewire_in_progress
echo "****************************************************************************"
...
echo
echo 10 > /tmp/dependancy_onewire_in_progress
echo "******************************************************************"
...
echo 30 > /tmp/dependancy_onewire_in_progress

echo "********************************************************"
echo "*             Installation des dependances       *"
echo "********************************************************"
sudo apt-get update
sudo  apt-get -y install automake autoconf autotools-dev gcc-4.7 libtool libusb-dev libfuse-dev swig python2.7-dev tcl8.5-dev php5-dev i2c-tools php7.0-bcmath
echo 50 > /tmp/dependancy_onewire_in_progress
...
echo 70 > /tmp/dependancy_onewire_in_progress
