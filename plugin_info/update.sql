IF NOT EXISTS (SELECT *  FROM information_schema.columns  WHERE table_name = 'onewire' AND column_name = 'class2')
BEGIN
ALTER TABLE onewire ADD COLUMN `class2` LONGTEXT NULL;
END;
