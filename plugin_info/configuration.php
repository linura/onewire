...
<script>
   $('.bt_installUSB').on('click', function () {
        $.ajax({// fonction permettant de faire de l'ajax
            ...
            $('#div_alert').showAlert({message: '{{Le Script du Dongle USB est en cours d\'installation. Voir le log onewire_dependancy }}', level: 'success'});
        }
    });
    });
       $('.bt_installUSB').on('click', function () {
        $.ajax({// fonction permettant de faire de l'ajax
            ...
            $('#ul_plugin .li_plugin[data-plugin_id=onewire]').click();   // recharge la page config du plugin
            $('#div_alert').showAlert({message: '{{Le Script du Gpio est en cours d\'installation. Voir le log onewire_dependancy }}', level: 'success'});
        }
    });
    });
       $('.bt_installUSB').on('click', function () {
        $.ajax({// fonction permettant de faire de l'ajax
            ...
            $('#ul_plugin .li_plugin[data-plugin_id=onewire]').click();   // recharge la page config du plugin
            $('#div_alert').showAlert({message: '{{Le Script du I2c est en cours d\'installation. Voir le log onewire_dependancy }}', level: 'success'});
        }
    });
    });
    </script>
