...
<script>
   ...
   if(sensor_class !=''  && sensor_id!='')
        configureDeviceLoad(sensor_id, sensor_class);
            ...
            $('#div_configureDeviceAlert').showAlert({message: '{{Erreur les parametres ne sont pas envoyés, merci de remplir tous les champs .}}', level: 'danger'});
    });
...
function sendParameter(param,valeur,sensor_id,equ_id){
    $.ajax({// fonction permettant de faire de l'ajax
        ...
        success: function(data) { // si l'appel a bien fonctionné
            ...
            $('#div_configureDeviceAlert').showAlert({message: 'Parametre envoyé', level: 'success'});
        }
    });
